...
function TourCard({ tour }) {
  return (
    <div className="rounded-[3px] overflow-hidden shadow-[0_1.5rem_4rem_rgba(0,_0,_0,_0.1)] bg-white transition-all ease-[ease] duration-300 backface-hidden flex flex-col">
      ...
      <div className="grid grid-cols-[1fr_1fr] gap-y-7 gap-x-8 py-10 px-12">
      ...
      <div className="bg-[#f7f7f7] [border-top:1px_solid_#f1f1f1] text-[1.4rem] grid grid-cols-[auto_1fr] gap-x-4 gap-y-4 mt-auto py-10 px-12">
        <p>
        ...
        <Link
          href={`/tour/${tour.slug}`}
          className="row-start-1 row-end-3 justify-self-end self-center text-[1.4rem]! rounded-[10rem] uppercase inline-block no-underline relative transition-all ease-[ease] duration-400 font-normal backface-hidden border-0 cursor-pointer bg-[#55c57a] text-white py-5! px-12! hover:[transform:translateY(-3px)] hover:shadow-[0_1rem_2rem_rgba(0,_0,_0,_0.15)] active:[transform:translateY(-1px)] active:shadow-[0_0.5rem_1rem_rgba(0,_0,_0,_0.15)] focus:outline-none focus:bg-[#2e864b]"
        >
          Details
        </Link>
      </div>
